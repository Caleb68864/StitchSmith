...
export function PatternPreview({ inputs, hasErrors }: Props) {
  const result = useMemo<{ svg: string; pieceCount: number; buildError: string | null } | null>(() => {
    ...
    try {
      ...
      if (!r.ok) return { svg: '', pieceCount: 0, buildError: r.errors[0]?.message ?? 'Build failed' };
      const pattern: Pattern = { id: 'zip-pouch', name: 'Zip Pouch', pieces: r.value.pieces };
      const svg = patternToSvg(pattern, {
        defaultSeamAllowance: inputs.seam_allowance ?? 10,
        showLabels: true,
      });
      return { svg, pieceCount: pattern.pieces.length, buildError: null };
    } catch (e) {
