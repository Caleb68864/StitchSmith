...
import { buildPattern } from '../../../generators/mag-pouch/buildPattern.js';

function makeProps() {
  const project = makeDefaultMagPouchProject();
  return { inputs: project.inputs, project, result: buildPattern(project.inputs), hasErrors: false };
}

const download = downloadTextFile as unknown as ReturnType<typeof vi.fn>;

describe('ExportPanel (mag-pouch) — export failures are surfaced, not swallowed', () => {
  beforeEach(() => {
    pdfLoader.mockReset();
    dxfLoader.mockReset();
